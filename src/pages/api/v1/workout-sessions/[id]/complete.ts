 ...
 * Validates that all session sets are completed or skipped (no pending sets).
 *
 * Path parameters:
 ...
 * Errors: 400 (invalid status), 401 (unauthorized), 404 (not found), 422 (pending sets), 500 (server error)
 */
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    // Call service to complete session
    ...
    logApiError(`POST /api/v1/workout-sessions/${context.params.id}/complete`, userId || undefined, error, "error");
